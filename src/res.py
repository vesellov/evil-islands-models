...
def read_model_data(figures_res_file_path, model_name, dest_parent_dir, save_json=False):
    dest_dir = os.path.join(dest_parent_dir, model_name)
    ...
    with open(figures_res_file_path, 'rb') as figures_file:
        ...
        if res_mod_element:
            mod_file_name = unpack_res_element(figures_file, res_mod_element, dest_file_name=os.path.join(dest_dir, model_name + '.mod'))
            mod_filetree = unpack_mod_info(mod_file_name, destination_dir=dest_dir)
            for mod_element in mod_filetree:
                if mod_element[0].endswith('.fig'):
                    fig_file_name = os.path.join(dest_dir, mod_element[0])
                    data['figures'][mod_element[0]] = read_fig_info(fig_file_name)
                    fig_count += 1
        ...
        if _Debug:
            print(f'loaded {fig_count} figures')
            print(f'loaded {lnk_count} links')
        res_anm_element = res_filetree_dict.get(model_name + '.anm')
        if res_anm_element:
            anm_file_name = unpack_res_element(figures_file, res_anm_element, dest_file_name=os.path.join(dest_dir, model_name + '.anm'))
            with open(anm_file_name, 'rb') as anm_file:
                anm_filetree = read_res_filetree(anm_file)
                for anm_element in anm_filetree:
                    if not os.path.isdir(os.path.join(dest_dir, anm_element[0])):
                        os.makedirs(os.path.join(dest_dir, anm_element[0]))
                    anm_element[0] += '.anm'
                unpack_res(anm_file, anm_filetree, destination_dir=dest_dir)
                for anm_element in anm_filetree:
                    one_anm_file_name = os.path.join(dest_dir, anm_element[0])
                    with open(one_anm_file_name, 'rb') as one_anm_file:
                        one_anm_filetree = read_res_filetree(one_anm_file)
                        data['animations'][anm_element[0]] = {}
                        for one_anm_element in one_anm_filetree:
                            one_anm_dest_file_name = os.path.join(dest_dir, anm_element[0][:-4], one_anm_element[0] + '.anm')
                            unpack_res_element(one_anm_file, one_anm_element, dest_file_name=one_anm_dest_file_name)
                            data['animations'][anm_element[0]][one_anm_element[0]] = read_anm_info(one_anm_dest_file_name)
                            anm_count += 1
        ...
        if res_bon_element:
            ...
            with open(bon_file_name, 'rb') as bon_file:
                bon_filetree = read_res_filetree(bon_file)
                for bon_element in bon_filetree:
                    bon_element[0] += '.bon'
                unpack_res(bon_file, bon_filetree, destination_dir=dest_dir)
                for bon_element in bon_filetree:
                    one_bon_file_name = os.path.join(dest_dir, bon_element[0])
                    data['bones'][bon_element[0]] = read_bon_info(one_bon_file_name)
                    bon_count += 1
